...
const iconMap = {
  ...
  Lock: <Lock className="text-red-600 dark:text-red-300" size={24} />,
  Layers: <Layers className="text-orange-600 dark:text-orange-300" size={24} />,
  FileCode: <FileCode className="text-green-600 dark:text-green-300" size={24} />,
  Code: <Code className="text-purple-600 dark:text-purple-300" size={24} />,
};

const Modifiers = () => {
  const { data: cards, loading, error } = useContent("modifiers");
  ...
  return (
    <div className="min-h-screen w-full py-8 px-4">
      ...
      <div className="mb-12">
        <img src={modifierImg} alt="Modifiers Image" className="w-full rounded-lg shadow-md" />
      </div>
        ...
        {cards.map((card) => (
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              {iconMap[card.icon]}
              <h2 className="text-2xl font-semibold text-gray-900 dark:text-white">{card.title}</h2>
            </div>

            {card.description && (
              <p className="text-gray-700 dark:text-gray-300 mb-5 leading-relaxed">{card.description}</p>
            )}
